...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
    ...
    Class.prototype.pullData = function (curr, loadIndex) {
        ...
        if (options.url) { //Ajax请求
            ...
            layui.set(res, response.dataName, options.data.concat().splice(startLimit, options.limit));
            layui.set(res, response.countName, options.data.length);
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
        var that = this
            , options = that.config
            , data = layui.get(res, options.response.dataName) || []
            , trs = []
            , trs_fixed = []
            , trs_fixed_r = []
    ...
    Class.prototype.sort = function (th, type, pull, formEvent) {
        ...
        try {
            ...
            if (sorts) {
                ...
                $(sorts).each(function () {
                    // console.log(this);
                    // var sort = this;
                    var elemSort = that.layHeader.find('th .laytable-cell-' + options.index + '-' + this.name).find(ELEM_SORT);

                    elemSort.attr('lay-sort', this.order || null);
                });
            }
        ...
        layui.set(res, options.response.dataName, thisData);
